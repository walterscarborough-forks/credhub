...
public class SignedCertificateGeneratorTest {
  ...
  public void beforeEach() throws Exception {
    ...
    x509v3CertificateBuilder
        .addExtension(Extension.subjectKeyIdentifier, false, caSubjectKeyIdentifier);
    certificateAuthorityWithSubjectKeyId = createCertificateAuthority(x509v3CertificateBuilder);
  }

  private CertificateCredentialValue createCertificateAuthority(
      X509v3CertificateBuilder x509v3CertificateBuilder)
      throws OperatorCreationException, CertificateException, IOException {
    X509CertificateHolder certificateHolder = x509v3CertificateBuilder
        .build(jcaContentSignerBuilder.build(issuerKey.getPrivate()));
    X509Certificate x509CertificateAuthority = jcaX509CertificateConverter
        .getCertificate(certificateHolder);
    expectedSubjectKeyIdentifier = x509CertificateAuthority
  ...
  public void getSelfSigned_generatesACertificateWithTheRightValues() throws Exception {
    X509Certificate generatedCertificate = subject
        .getSelfSigned(generatedCertificateKeyPair, certificateParameters);
    ...
    byte[] authorityKeyIdDer = generatedCertificate
    ...
    AuthorityKeyIdentifier authorityKeyIdentifier = AuthorityKeyIdentifier
        .getInstance(parseExtensionValue(authorityKeyIdDer));
    byte[] authorityKeyId = authorityKeyIdentifier.getKeyIdentifier();

    expectedSubjectKeyIdentifier = jcaX509ExtensionUtils
        .createSubjectKeyIdentifier(generatedCertificateKeyPair.getPublic()).getKeyIdentifier();
    ...
    assertThat(authorityKeyIdentifier.getAuthorityCertSerialNumber(),
        equalTo(BigInteger.valueOf(1337)));
  }

  @Test
  public void getSignedByIssuer_generatesACertificateWithTheRightValues() throws Exception {
    X509Certificate generatedCertificate = subject
        .getSignedByIssuer(generatedCertificateKeyPair, certificateParameters,
    ...
    assertThat(generatedCertificate.getNotBefore().toString(),
        equalTo(Date.from(now.toInstant()).toString()));
    assertThat(generatedCertificate.getNotAfter().toString(),
        equalTo(Date.from(later.toInstant()).toString()));
    assertThat(generatedCertificate.getSubjectDN().toString(), containsString("CN=my cert name"));
    assertThat(generatedCertificate.getPublicKey(),
        equalTo(generatedCertificateKeyPair.getPublic()));
    assertThat(generatedCertificate.getSigAlgName(), equalTo("SHA256WITHRSA"));
    generatedCertificate.verify(issuerKey.getPublic());

    byte[] isCaExtension = generatedCertificate
  ...
  @Test
  public void getSignedByIssuer_withoutSubjectKeyIdentifier_doesNotSetAuthorityKeyIdentifier()
      throws Exception {
    X509Certificate generatedCertificate =
        subject.getSignedByIssuer(generatedCertificateKeyPair, certificateParameters,
            certificateAuthority);

    assertThat(generatedCertificate.getExtensionValue(Extension.authorityKeyIdentifier.getId()),
        nullValue());
  }

  @Test
  public void getSignedByIssuer_withSubjectKeyIdentifier_setsAuthorityKeyIdentifier()
      throws Exception {
    ...
    assertThat(authorityKeyIdentifier.getKeyIdentifier(),
        equalTo(caSubjectKeyIdentifier.getKeyIdentifier()));
    assertThat(authorityKeyIdentifier.getAuthorityCertSerialNumber(), equalTo(caSerialNumber));
    assertThat(authorityKeyIdentifier.getAuthorityCertIssuer().getNames()[0].getName().toString(),
        equalTo(new X500Principal(caName).getName()));
  }
  ...
  public void getSignedByIssuer_setsAlternativeName_ifPresent() throws Exception {
    ...
    assertThat(generatedCertificate.getExtensionValue(Extension.subjectAlternativeName.getId()),
        nullValue());
    ...
    byte[] actualSubjectAlternativeName = generatedCertificate
        .getExtensionValue(Extension.subjectAlternativeName.getId());
    byte[] expectedAlternativeName = getExpectedAlternativeNames();
    assertThat(
        Arrays.copyOfRange(actualSubjectAlternativeName, 2, actualSubjectAlternativeName.length),
  ...
  public void getSignedByIssuer_setsExtendedKeyUsage_ifPresent() throws Exception {
    ...
    generatedCertificate = subject
        .getSignedByIssuer(generatedCertificateKeyPair, certificateParameters,
            certificateAuthorityWithSubjectKeyId);
    byte[] actualKeyUsage = generatedCertificate
  ...
  public void getSignedByIssuer_preservesIssuerBytes() throws Exception {
    ...
    X509Certificate generatedCertificate = subject
        .getSignedByIssuer(generatedCertificateKeyPair, certificateParameters, ca);

    assertThat(generatedCertificate.getIssuerX500Principal().getEncoded(),
        equalTo(caCertificate.getSubjectX500Principal().getEncoded()));
  }
